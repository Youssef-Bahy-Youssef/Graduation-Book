...
\begin{description}
  ...
  \item[\texttt{TRANSCODING}.] The instructor has initiated an upload and Mux has confirmed receipt and begun transcoding. The NexaLearn backend creates a \texttt{GenerationJob} entity for the AI pipeline (Section~\ref{sec:ai-elearning}) in a pending state, awaiting the completed transcript.
  \item[\texttt{READY}.] Mux sends \texttt{video.asset.ready}, indicating that all HLS renditions are available. NexaLearn updates the \texttt{VideoAsset} with the playback ID and signed URL expiration policy. The AI pipeline receives a \texttt{VideoTranscodingCompleted} event via the outbox, triggering the transcript-generation workflow.
  \item[\texttt{ERROR}.] Transcoding failed. NexaLearn logs the Mux error code and message, notifies the instructor, and makes the upload available for retry.
\end{description}
