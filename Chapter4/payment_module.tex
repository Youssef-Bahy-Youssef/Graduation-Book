...
\begin{enumerate}
  \item The client requests \texttt{POST /payments/intent} with course ID and learner ID. The Payment module creates a Stripe Payment Intent via the Stripe SDK and returns the client secret to the client.
  \item The client completes payment on Stripe's hosted checkout page. The learner completes payment on Stripe's domain, which reduces PCI-DSS compliance scope for the platform.
  \item Stripe sends a \texttt{checkout.session.completed} webhook to NexaLearn's \texttt{POST /payments/webhook}. The webhook handler verifies the Stripe signature using \texttt{stripe.webhooks.constructEvent()}, retrieves the session details, and invokes the \texttt{HandlePaymentSucceededUseCase}.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.65\textwidth]{images/statemachines/payment_intent_statemachine.pdf}
  \caption{Payment Intent State Machine}
  \label{fig:payment-statemachine}
\end{figure}

The \texttt{PaymentIntent} aggregate transitions through states: CREATED when the payment is initiated, PENDING while awaiting Stripe confirmation, SUCCEEDED upon confirmed payment, FAILED if the payment was declined or expired, and REFUNDED when a succeeded payment is refunded. Each transition is recorded as an immutable \texttt{PaymentEvent} in a separate event table, providing an audit trail for financial reconciliation.
...
\begin{description}
  \item[\textbf{ReconciliationService.}] Runs daily and retrieves all PaymentIntents in PENDING state older than 24 hours. It queries Stripe for the latest status of each associated Payment Intent and updates the local state accordingly, handling cases where the webhook was never delivered (e.g., network outage).
  \item[\textbf{ExpirationService.}] Runs hourly and expires Payment Intents older than 24 hours. This garbage-collects abandoned payment flows and releases any temporary holds on inventory or enrollment slots.
\end{description}
