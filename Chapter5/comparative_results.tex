...
\begin{table}[H]
  ...
  \begin{tabular}{lcccc}
    ...
    \midrule
    Test framework                  & Jest                 & PHPUnit             & PHPUnit            & Pytest + tox             \\
    Language                        & TypeScript           & PHP                 & PHP                & Python                   \\
    Total test cases                & 396                  & $\sim$8,400         & $\sim$6,200        & $\sim$4,100             \\
    Unit test ratio                 & 68\%                 & 52\%                & 48\%               & 55\%                     \\
    Integration test ratio          & 23\%                 & 31\%                & 35\%               & 28\%                     \\
    E2E test ratio                  & 9\%                  & 17\%                & 17\%               & 17\%                     \\
    \midrule
    \textbf{Infrastructure}         &                      &                     &                    \\
    Real DB in integration tests    & Yes (Testcontainers) & Yes (PHPUnit DB)    & Yes (PHPUnit DB)   & Partial (sqlite in-memory)\\
    Containerised test environment  & Yes                  & Partial             & Partial            & Yes (Docker Compose)     \\
    CI parallelisation              & 4 shards             & 8 shards            & 4 shards           & 6 shards                 \\
    Coverage threshold (enforced)   & 90\% entity / 80\% svc. & 80\% (PHP projects)& 75\% (PHP projects)& 80\% (Python projects)\\
    \midrule
    \textbf{DDD-Specific Testing}   &                      &                     &                    \\
    Aggregate invariant tests       & Yes (all aggregates) & Partial             & Partial            & No                       \\
    State machine enforcement       & Yes (code-level)     & No                  & No                 & No                       \\
    ...
    Average CI time (full suite)    & 4 min 12 s           & 22 min              & 18 min             & 35 min                   \\
    Test warm-up (cold cache)       & 8 s (Testcontainers) & 12 s (DB setup)     & 12 s (DB setup)    & 45 s (Docker Compose)    \\
    Watch-mode support              & Yes (Jest --watch)   & Partial (PHPUnit)   & Partial (PHPUnit)  & No (pytest-watch 3rd-party)\\
    \bottomrule
  \end{tabular}
\end{table}
