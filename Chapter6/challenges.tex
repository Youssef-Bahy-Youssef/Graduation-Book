...
\textbf{Solution.} The webhook handler wraps enrollment creation in the \texttt{tryBegin\-Projection\-Event()} pattern from Section~\ref{sec:challenges}. Additionally, the \texttt{PaymentIntent} aggregate enforces a state machine invariant: a payment intent in \texttt{SUCCEEDED} state cannot transition to \texttt{SUCCEEDED} again. This double layer of protection---database-level idempotency and domain-level state machine---ensures that duplicate webhooks cannot produce incorrect state.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\textwidth]{images/video_rendition_statemachine.pdf}
